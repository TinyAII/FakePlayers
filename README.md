...
> Tab 假人 + 随机皮肤 + 假 Ping + MOTD 虚高 + 孤独搭话 + 随机上下线 + 防重名 + 称号前缀，零依赖撑排面。MIT 开源。

小服"撑排面"利器：给 Tab 列表塞假人（随机英文 ID + 内置 23 张皮肤 + 假 Ping），MOTD 在线人数虚高让小服看起来热闹；单人在线时假人随机闲谈台词缓解冷清；假人还会随机上下线配合 join 提示；支持称号前缀（全局前缀 + 单角色前缀）。
  ...
  *  称号前缀：支持全局前缀 + 单角色前缀，前缀带颜色、名字保持默认色；兼容 & 颜色代码
  *  品牌横幅 TinyAII；MIT 开源

## 安装

  1. 下载 fakeplayers-1.1.0.jar
  2. 放入 plugins/，重启
...
fake-players:
  count: 2                 # 假人数量
  enabled: true            # 是否启用
  prefix: ""               # 全局称号前缀（支持 & 颜色代码，如 &c[VIP] ）
  custom-prefixes:         # 单角色前缀（按名字单独配，优先于 prefix）
    zhangsan: "&c[VIP] "
    lisi: "&a[LV] "
  ping:
    min: 1
...
- prefix：全局前缀，所有假人共用。颜色代码用 & 开头，如 &c[VIP] 、&b[Member] 。
- custom-prefixes：按假人名字单独配置前缀，优先级高于全局 prefix。
- 前缀带颜色，名字本身保持默认白色（自动重置颜色）。
- 支持多行颜色代码，如 &l&n[VIP]&r （加粗+下划线）。
...
Fake players for Tab list with random skins, fake ping, MOTD inflation, lonely-chat, random join/quit, name prefix. MIT open source, zero deps.
...
## Features
  ...
  * Name prefix (global + per-fake, color code & supported, name color reset to default)

## Commands

/fakeplayer count N, /fp on|off, /fp reload, /fp list. Alias: /假人, /jr. Permission: fakeplayers.admin.
